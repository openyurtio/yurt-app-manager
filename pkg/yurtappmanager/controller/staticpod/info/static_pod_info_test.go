...
func Test_ConstructStaticPodsUpgradeInfoList(t *testing.T) {
	...
	expect := map[string]*StaticPodInfo{
		"node1": {
	...
	t.Run("test", func(t *testing.T) {
		spi, _ := ConstructStaticPodsUpgradeInfoList(c, newStaticPod(), UpgradeWorkerPodPrefix)
...
func TestNodes(t *testing.T) {
	tHash := "tHash"
	fHash := "fHash"
	spi := map[string]*StaticPodInfo{
		"node1": {
			UpgradeExecuting: true,
			UpgradeNeeded:    true,
			Ready:            true,
		},
		"node2": {
