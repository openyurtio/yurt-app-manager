...
func (r *ReconcileUnitedDeployment) managePools(ud *unitv1alpha1.UnitedDeployment,
	...
	if err != nil {
		SetUnitedDeploymentCondition(newStatus, NewUnitedDeploymentCondition(unitv1alpha1.UnitedPoolProvisioned, corev1.ConditionFalse, "Error", err.Error()))
		return newStatus, fmt.Errorf("fail to manage Pool provision: %s", err)
	}

	if provisioned {
		SetUnitedDeploymentCondition(newStatus, NewUnitedDeploymentCondition(unitv1alpha1.UnitedPoolProvisioned, corev1.ConditionTrue, "", ""))
	}
	...
	if updateErr == nil {
		SetUnitedDeploymentCondition(newStatus, NewUnitedDeploymentCondition(unitv1alpha1.UnitedPoolUpdated, corev1.ConditionTrue, "", ""))
	} else {
		SetUnitedDeploymentCondition(newStatus, NewUnitedDeploymentCondition(unitv1alpha1.UnitedPoolUpdated, corev1.ConditionFalse, "Error", updateErr.Error()))
	}
	return
}
