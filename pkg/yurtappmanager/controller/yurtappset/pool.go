...
type ControlInterface interface {
	...
	GetAllPools(ud *unitv1alpha1.YurtAppSet) ([]*Pool, error)
	// CreatePool creates the pool depending on the inputs.
	CreatePool(ud *unitv1alpha1.YurtAppSet, unit string, revision string, replicas int32) error
	// UpdatePool updates the target pool with the input information.
	UpdatePool(pool *Pool, ud *unitv1alpha1.YurtAppSet, revision string, replicas int32) error
