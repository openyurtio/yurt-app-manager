...
func (m *PoolControl) GetAllPools(ud *alpha1.YurtAppSet) (pools []*Pool, err error) {
...
func (m *PoolControl) CreatePool(ud *alpha1.YurtAppSet, poolName string, revision string,
	replicas int32) error {

	set := m.adapter.NewResourceObject()
	m.adapter.ApplyPoolTemplate(ud, poolName, revision, replicas, set)

	klog.V(4).Infof("Have %d replicas when creating Pool for YurtAppSet %s/%s", replicas, ud.Namespace, ud.Name)
...
func (m *PoolControl) UpdatePool(pool *Pool, ud *alpha1.YurtAppSet, revision string, replicas int32) error {
	set := m.adapter.NewResourceObject()
	cliSet, ok := set.(client.Object)
	if !ok {
		return errors.New("fail to convert runtime.Object to client.Object")
