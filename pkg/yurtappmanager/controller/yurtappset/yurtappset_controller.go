...
func Add(mgr manager.Manager, _ context.Context) error {
	if !gate.ResourceEnabled(&unitv1alpha1.YurtAppSet{}) {
		return nil
	}
...
func (r *ReconcileYurtAppSet) updateYurtAppSet(ud *unitv1alpha1.YurtAppSet, oldStatus, newStatus *unitv1alpha1.YurtAppSetStatus) (*unitv1alpha1.YurtAppSet, error) {
	...
	klog.Errorf("fail to update YurtAppSet %s/%s status: %s", ud.Namespace, ud.Name, updateErr)
