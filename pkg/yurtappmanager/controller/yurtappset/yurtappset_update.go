...
func (r *ReconcileYurtAppSet) managePools(ud *unitv1alpha1.YurtAppSet,
...
func (r *ReconcileYurtAppSet) managePoolProvision(ud *unitv1alpha1.YurtAppSet,
	nameToPool map[string]*Pool, nextPatches map[string]YurtAppSetPatches,
	...
	klog.V(4).Infof("YurtAppSet %s/%s has pools %v, expects pools %v", ud.Namespace, ud.Name, gotPools.List(), expectedPools.List())
	...
	if len(creates) > 0 {
		...
		klog.Infof("YurtAppSet %s/%s needs creating pool (%s) with name: %v", ud.Namespace, ud.Name, workloadType, creates)
	...
	// manage deleting
	if len(deletes) > 0 {
		klog.Infof("YurtAppSet %s/%s needs deleting pool (%s) with name: [%v]", ud.Namespace, ud.Name, workloadType, deletes)
	...
	for t, control := range r.poolControls {
		...
		if err != nil {
			errs = append(errs, fmt.Errorf("fail to list Pool of other type %s for YurtAppSet %s/%s: %s", t, ud.Namespace, ud.Name, err))
			continue
		}
		...
		for _, pool := range pools {
			...
			if err := control.DeletePool(pool); err != nil {
				errs = append(errs, fmt.Errorf("fail to delete Pool %s of other type %s for YurtAppSet %s/%s: %s", pool.Name, t, ud.Namespace, ud.Name, err))
				continue
			}
		}
	}
