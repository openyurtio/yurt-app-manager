...
func (r *YurtIngressReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&appsv1alpha2.YurtIngress{}, builder.WithPredicates(
...
func (r *YurtIngressReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Client.Get(context.TODO(), req.NamespacedName, yurtIngress); err != nil {
		if !errors.IsNotFound(err) {
			klog.Errorf("get yurtingress %v failed, error: %v", yurtIngressName, err)
			return ctrl.Result{Requeue: true}, err
		}
	...
	if !nodePoolExist && !yurtIngressExist {
		...
		if err := r.Client.Delete(context.TODO(), yurtIngress); err != nil {
			klog.Errorf("delete yurtingress %v failed, error: %v", yurtIngressName, err)
			return ctrl.Result{Requeue: true}, err
		}
		klog.Infof("nodepool not exist, yurtingress delete succ, %s", req.Name)
		return ctrl.Result{}, nil
		...
		klog.V(4).Infof("nodepool and yurtingress all exists, %s", nodePool.Name)
	}

	if !controllerutil.ContainsFinalizer(yurtIngress, appsv1alpha2.YurtIngressFinalizer) {
		patchYi := yurtIngress.DeepCopy()
		controllerutil.AddFinalizer(patchYi, appsv1alpha2.YurtIngressFinalizer)
		if err := r.Patch(context.TODO(), patchYi, client.MergeFrom(yurtIngress)); err != nil {
			return ctrl.Result{Requeue: true}, err
		}
	}

	if !yurtIngress.Spec.Enabled {
		klog.Infof("yurtingress %s disabled, skip process", yurtIngress.Name)
		return ctrl.Result{}, nil
	}

	if err := r.reconcileTopologyService(ctx, yurtIngress); err != nil {
		return ctrl.Result{}, err
	}

	result, err := r.reconcileYurtIngress(ctx, yurtIngress, nodePool)
