...
func NewRunner(getter genericclioptions.RESTClientGetter, storageNamespace string, log logr.Logger) (*Runner, error) {
	runner := &Runner{
		logBuffer: NewLogBuffer(NewDebugLog().Log, defaultBufferSize),
	}
...
func (r *Runner) ObserveLastRelease(releaseName string) (*release.Release, error) {
	rel, err := r.config.Releases.Last(releaseName)
