...
func Ensure(c client.Client, handlers map[string]webhookutil.Handler, caBundle []byte) error {
	...
	var mutatingWHs []v1.MutatingWebhook
	...
	var validatingWHs []v1.ValidatingWebhook
...
func getPath(clientConfig *v1.WebhookClientConfig) (string, error) {
...
func convertClientConfig(clientConfig *v1.WebhookClientConfig, host string, port int) {
	url := fmt.Sprintf("https://%s:%d%s", host, port, *clientConfig.Service.Path)
	clientConfig.URL = &url
	clientConfig.Service = nil
}

func parseMutatingTemplate(mutatingConfig *v1.MutatingWebhookConfiguration) ([]v1.MutatingWebhook, error) {
...
func parseValidatingTemplate(validatingConfig *v1.ValidatingWebhookConfiguration) ([]v1.ValidatingWebhook, error) {
