...
func (h *YurtAppSetCreateUpdateHandler) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	klog.Infoln("YurtAppSetCreateUpdateHandler")
	klog.Infoln(string(req.Object.Raw))
	klog.Infoln(string(origin))

	unitv1alpha1.SetDefaultsYurtAppSet(obj)
	obj.Status = unitv1alpha1.YurtAppSetStatus{}
	...
	klog.Infoln("after YurtAppSetCreateUpdateHandler")
	...
	if len(resp.Patches) > 0 {
		klog.V(5).Infof("Admit YurtAppSet %s/%s patches: %v", obj.Namespace, obj.Name, util.DumpJSON(resp.Patches))
	}
	return resp
}
